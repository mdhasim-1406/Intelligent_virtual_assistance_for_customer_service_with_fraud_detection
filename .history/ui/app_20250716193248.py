...
def check_system_health():
    ...
    try:
        ...
        # Get capabilities
        cap_response = requests.get(f"{st.session_state.api_url}/capabilities", timeout=5)
        cap_data = cap_response.json() if cap_response.status_code == 200 else {}
        
        return health_data, cap_data
...
def send_message(text: str, language: str = "auto", mode: str = "text", audio: str = None):
    """Send message to SafeServe AI API"""
    try:
        payload = {
            "text": text,
            "lang": language,
...
def display_fraud_indicator(fraud_likelihood: float, fraud_label: str, risk_factors: list):
    """Display fraud detection results"""
    if fraud_label == "high_risk":
        st.markdown(f"""
        <div class="fraud-alert">
            <h4>🚨 HIGH FRAUD RISK DETECTED</h4>
            <p><strong>Risk Score:</strong> {fraud_likelihood:.2%}</p>
        ...
        <div class="warning-indicator">
            ...
            <p><strong>Risk Score:</strong> {fraud_likelihood:.2%}</p>
            <p><strong>Risk Factors:</strong> {', '.join(risk_factors)}</p>
            <p><em>Please verify customer identity before proceeding.</em></p>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown(f"""
                ...
                with st.spinner("Processing your message..."):
                    ...
                    if response:
                        ...
                        st.session_state.chat_history.append({
                            'user': user_input,
                            'assistant': response['translated_response'],
                            ...
                            'risk_factors': response['capabilities'].get('fraud_factors', []),
