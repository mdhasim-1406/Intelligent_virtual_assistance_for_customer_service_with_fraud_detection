...
def send_message(text: str, lang: str = "en", mode: str = "text", audio: str = None) -> dict:
    """
    Unified send_message function that handles both text and voice interactions
    This is the main interface function that the UI components call
    """
    try:
        if mode == "voice" and audio:
            # For voice input, we'll use the assistant endpoint when available
            ...
            # Try assistant endpoint first (if available)
            try:
                response = requests.post(
                    f"{st.session_state.api_url}/assistant",
                    json=payload,
                    timeout=30
                )
            ...
            return send_chat_message(text, lang)
        else:
            # Regular text chat
            return send_chat_message(text, lang)
            
    except Exception as e:
        st.error(f"Send message failed: {str(e)}")
        return None
...
with tab3:
    ...
    if st.session_state.chat_history:
        ...
        with col3:
            high_risk_count = sum(1 for chat in st.session_state.chat_history if chat['fraud_label'] == 'Suspicious')
...
with tab4:
    ...
    # Test scenarios
    st.subheader("Chat Assistant Test Scenarios")
    ...
    with col1:
        if st.button("Run Test", type="primary"):
            with st.spinner("Running test..."):
                response = send_chat_message(st.session_state.test_message)
                ...
                if response:
                    ...
                    with col2:
                        st.write("**Analysis:**")
                        st.write(f"Processing Time: {response.get('processing_time', 0):.2f}s")
                        st.write(f"User ID: {response.get('user_id', 'Unknown')}")
                        st.write(f"Timestamp: {response.get('timestamp', 'Unknown')}")
    ...
    with col2:
        if st.button("Batch Test All Scenarios"):
            ...
            if results:
                ...
                # Summary statistics
                st.write("**Summary:**")
                st.write(f"Average Response Length: {results_df['response_length'].mean():.0f} characters")
                st.write(f"Average Processing Time: {results_df['processing_time'].mean():.2f}s")
                st.write(f"Total Scenarios Tested: {len(results)}")
    
    # Fraud Detection Testing
    st.subheader("Fraud Detection Test")
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.write("**Transaction Details:**")
        test_amount = st.number_input("Amount ($)", min_value=0.01, value=1000.0, step=0.01)
        test_location = st.text_input("Location", value="New York, NY")
        test_merchant = st.text_input("Merchant", value="Amazon")
        test_device_id = st.text_input("Device ID", value="device_123")
        test_velocity = st.slider("Velocity Score", 0.0, 1.0, 0.5, 0.01)
        
        if st.button("Test Fraud Detection"):
            with st.spinner("Analyzing transaction..."):
                fraud_response = send_fraud_prediction(
                    test_amount, test_location, test_merchant, 
                    test_device_id, test_velocity
                )
                ...
                if fraud_response:
                    ...
                    display_fraud_indicator(
                        ...
                        fraud_response['label']
                    )
                    st.write(f"**Explanation:** {fraud_response['explanation']}")
    
    with col2:
        st.write("**Combined Analysis Test:**")
        
        if st.button("Test Combined Analysis"):
            with st.spinner("Running combined analysis..."):
                combined_response = send_combined_analysis(
                    test_amount, test_location, test_merchant,
                    test_device_id, test_velocity
                )
                ...
                if combined_response:
                    ...
                    fraud_data = combined_response['fraud_analysis']
                    display_fraud_indicator(
                    ...
                    st.write(combined_response['explanation'])