...
Integrates with Google Colab Deepseek LLM via ngrok with ALBERT fallback
"""
...
try:
    ...
    # Train with synthetic data if not already trained
...
# ALBERT-based local fallback
albert_model = None
albert_tokenizer = None

def initialize_albert_fallback():
    """Initialize ALBERT model for local fallback"""
    global albert_model, albert_tokenizer
    try:
        ...
        albert_model = AutoModelForSequenceClassification.from_pretrained(model_name)
        
        logger.info("✅ ALBERT fallback model initialized")
        return True
    except Exception as e:
        logger.warning(f"❌ Failed to initialize ALBERT fallback: {str(e)}")
        return False

def get_albert_fallback_response(query: str) -> str:
    """Generate response using local ALBERT model"""
    try:
        if albert_model is None or albert_tokenizer is None:
            return get_simple_fallback_response(query)
        
        # Simple few-shot prompt for customer service
        prompt = f"""You are a helpful customer service assistant. Provide a brief, helpful response to this customer query:

Customer: {query}
Assistant:"""
        
        inputs = albert_tokenizer(prompt, return_tensors="pt", padding=True, truncation=True, max_length=512)
        outputs = albert_model(**inputs)
        response = outputs.logits.argmax(dim=-1).item()
        
        # Map response to predefined answers (for demo purposes, using fixed responses)
        response_map = {
            0: "I'm sorry, I don't have information on that. Please contact our support.",
            1: "For account issues, please visit our account recovery page or contact support.",
            2: "We take fraud seriously. If you suspect fraud, please contact our fraud department immediately.",
            3: "Thank you for your inquiry. Please check our FAQ page for common questions.",
            4: "Your security is important to us. Please enable two-factor authentication for your account.",
            5: "For transaction issues, please provide the transaction ID and details to our support team."
        }
        
        return response_map.get(response, "I'm here to help! Please provide more details about your query.")
    
    except Exception as e:
        logger.error(f"ALBERT fallback error: {str(e)}")
        return get_simple_fallback_response(query)

def get_simple_fallback_response(query: str) -> str:
...
# Initialize ALBERT fallback model at startup
initialize_albert_fallback()
...
# Enhanced LLM response function with fallback
def get_llm_response_with_fallback(query: str, user_id: str = "user") -> tuple[str, bool]:
    """Get response from LLM with fallback capability"""
    fallback_used = False
...
Response:"""
        ...
        if response.status_code == 200:
            ...
            if "response" in response_data:
                return response_data["response"].strip(), fallback_used
            else:
                logger.warning("LLM API returned invalid response format")
                fallback_used = True
                return get_albert_fallback_response(query), fallback_used
        else:
...
async def not_found_handler(request, exc):
    return JSONResponse(status_code=404, content={"error": "Endpoint not found", "message": "Please check the API documentation"})
