...
class FraudDetectionEngine:
    ...
    def predict_fraud(self, transaction_data: Dict) -> Dict[str, Any]:
        """
        Predict fraud for a transaction with comprehensive analysis
        ...
        """
        try:
            # Get basic prediction from the model
            prediction = self.predict(transaction_data)
            
            # Extract additional transaction details for analysis
            amount = float(transaction_data.get('amount', 0))
            location = transaction_data.get('location', 'unknown')
            merchant = transaction_data.get('merchant', 'unknown')
            device_id = transaction_data.get('device_id', 'unknown')
            ...
            if location.lower() in ['foreign', 'unknown', 'high_crime_area']:
                risk_factors.append("High-risk location")
            elif location.lower() in ['new_location', 'atm']:
                risk_factors.append("Unfamiliar location")
            
            # Merchant-based risk factors
            if merchant.lower() in ['online_gambling', 'crypto_exchange', 'unknown']:
                risk_factors.append("High-risk merchant category")
            
            # Device-based risk factors
            if device_id in ['unknown', 'new_device']:
            ...
            fraud_likelihood = min(1.0, base_risk * risk_multiplier)
            
            # Generate recommendation
            if fraud_likelihood > 0.8:
                recommendation = "BLOCK - High fraud risk detected. Contact customer immediately."
            elif fraud_likelihood > 0.6:
                recommendation = "REVIEW - Moderate fraud risk. Additional verification recommended."
            elif fraud_likelihood > 0.4:
                recommendation = "MONITOR - Low-moderate risk. Monitor for patterns."
            else:
            ...
            return {
                ...
                "recommendation": "REVIEW - Unable to complete fraud analysis. Manual review required.",
                "model_used": self.model_type,
                "analysis_timestamp": datetime.datetime.now().isoformat(),
                "error": str(e)
            }
