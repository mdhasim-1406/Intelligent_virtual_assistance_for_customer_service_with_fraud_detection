...
class FraudDetectionEngine:
    ...
    def predict_fraud(self, transaction_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            # Use the existing predict method as base
            base_prediction = self.predict(transaction_data)
            
            # Extract additional features
            features = self._extract_transaction_features(transaction_data)
            
            # Get fraud indicators
            fraud_indicators = self._get_fraud_indicators(transaction_data, features)
            
            # Enhanced prediction with additional analysis
            fraud_probability = base_prediction['risk_score']
            ...
            return {
                ...
                'confidence': round(base_prediction['confidence'], 4),
    ...
    def _get_fraud_indicators(self, transaction_data: Dict[str, Any], features: Dict[str, Any]) -> List[str]:
        ...
        if location in ['foreign', 'unknown', 'high_crime_area']:
        ...
        if transaction_data.get('velocity_score', 0) > 3:
            indicators.append("High transaction frequency")
        
        if transaction_data.get('device_id') == 'unknown':
