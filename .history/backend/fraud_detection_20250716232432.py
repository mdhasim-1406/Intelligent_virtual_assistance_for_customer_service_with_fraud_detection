...
class FraudDetectionEngine:
    ...
    def predict_fraud(self, transaction_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            behavioral_analysis = self.behavioral_analyzer.analyze_transaction_behavior(transaction_data)
            
            # Combine features with behavioral analysis
            combined_features = {**features, **behavioral_analysis.get('features', {})}
            
            # Create feature vector
            feature_vector = self._create_feature_vector(combined_features)
            ...
            fraud_indicators = self._get_fraud_indicators(transaction_data, combined_features)
    ...
    def _extract_transaction_features(self, transaction_data: Dict[str, Any]) -> Dict[str, Any]:
        """Extract features from transaction data"""
        features = {}
        ...
        features['amount_normalized'] = min(amount / 10000, 1.0)  # Normalize to 0-1
        
        # Time features
        timestamp = transaction_data.get('timestamp')
        if timestamp:
            dt = datetime.datetime.fromisoformat(timestamp) if isinstance(timestamp, str) else timestamp
        ...
        # Location features
        features['merchant_category'] = transaction_data.get('merchant_category', 'unknown')
        features['location'] = transaction_data.get('location', 'unknown')
        
        # Transaction type features
        features['transaction_type'] = transaction_data.get('transaction_type', 'unknown')
        features['channel'] = transaction_data.get('channel', 'unknown')
        
        return features
    
    def _get_fraud_indicators(self, transaction_data: Dict[str, Any], features: Dict[str, Any]) -> List[str]:
        """Get list of fraud indicators for the transaction"""
        indicators = []
        ...
        if features.get('unusual_location'):
            indicators.append("Unusual location")
        
        if features.get('unusual_amount'):
            indicators.append("Unusual amount pattern")
        
        if features.get('high_frequency'):
            indicators.append("High transaction frequency")
        
        # Merchant indicators
        merchant_category = features.get('merchant_category', '').lower()
        if merchant_category in ['gambling', 'adult', 'cash_advance']:
